...
typedef void (^MediaClickHandler)(HXYPlayMediumViewType type, NSString *mediaUrl, NSString *imageUrl);
@interface MXImageView : UIView<HXYPlayMediumViewDelegate>
...
@property (nonatomic, copy) MediaClickHandler mediaHandler;
...
@property (nonatomic, assign) NSInteger  type;      //类型-1、图片，2、视频，3、音频
@property (nonatomic, strong) HXYPlayMediumView *mediaView;
@end

@implementation MXImageView

- (instancetype)initWithFrame:(CGRect)frame imageURL:(NSString*)url placeholderImage:(UIImage*)placeholder model:(MXImageModel *)model{
    if (self = [super initWithFrame:frame]) {
        ...
        switch (self.type) {
            ...
            {
                if (!self.mediaView) {
                    self.mediaView = [[HXYPlayMediumView alloc]initWithType:HXYPlayMediumViewTypeVideo];
                    self.mediaView.playWay = HXYPlayMediaWayInController;
                    self.mediaView.delegate = self;
                }
                [self addSubview:self.mediaView];
                self.mediaView.imageUrl = model.imageUrl;
                self.mediaView.mediumUrl = model.media;
            ...
            {
                if (!self.mediaView) {
                    self.mediaView = [[HXYPlayMediumView alloc]initWithType:HXYPlayMediumViewTypeAudio];
                    self.mediaView.playWay = HXYPlayMediaWayInController;
                    self.mediaView.delegate = self;
                }
                [self addSubview:self.mediaView];
...
{
    self.mediaHandler(type,mediaUrl,imageUrl);
}
...
- (void)setContents:(NSArray<MXImageModel *> *)contents
{
    _contents = contents;
    CGFloat dotWidth = 5;
    CGFloat dotMargin = 9;
    if (self.pageControlDotWidth) {
        dotWidth = self.pageControlDotWidth;
    }
    if (self.pageControlDotMargin) {
        dotMargin = self.pageControlDotMargin;
    }
    CGFloat controlWidth = dotWidth * contents.count + dotMargin * (contents.count - 1);
    self.pageControlView.frame = CGRectMake(self.width - controlWidth - 16, self.height-MXPageControlHeight, controlWidth, MXPageControlHeight);
    ...
    self.modelArray = [NSMutableArray arrayWithArray:_contents];
    if (_contents.count > 1) {
        //这里在最开始和最后多加一张图片，做循环滚动
        MXImageModel *firstModel = [_contents lastObject];
        MXImageModel *lastModel  = [_contents firstObject];
        [self.modelArray insertObject:firstModel atIndex:0];
        [self.modelArray insertObject:lastModel atIndex:self.modelArray.count];
    }
    [self initContents];
}
...
- (void)setupImageViews {
    ...
    for (NSInteger i = 0; i < self.imageCount; i++) {
        @autoreleasepool {
            MXImageView *imageView = [[MXImageView alloc]initWithFrame:CGRectMake(self.width*i, 0, self.width, self.height) imageURL:_mImageArray[i] placeholderImage:self.placeholderImage model:self.modelArray[i]];
            ...
            imageView.handler = ^(NSInteger index) {
                ...
                if ([self.delegate respondsToSelector:@selector(clickImageIndex:imgsViews:)]) {
                    [self.delegate clickImageIndex:index imgsViews:[self.imgViews copy]];
                }
            ...
            imageView.mediaHandler = ^(HXYPlayMediumViewType type,NSString *mediaUrl,NSString *imageUrl){
                if ([_delegate respondsToSelector:@selector(didChooseMedia:mediaUrl:imageUrl:)]) {
                    [_delegate didChooseMedia:type mediaUrl:mediaUrl imageUrl:imageUrl];
                }
            };
            [self.scrollView addSubview:imageView];
            [self.imageViewArray addObject:imageView];
            [self.imgViews addObject:imageView.imageView];
        }
    }
